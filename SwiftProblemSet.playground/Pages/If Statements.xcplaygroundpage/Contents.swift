...
func checkForRelease(bugs: Int, music: Bool, levels: Int) {
    if bugs <= 9 && hasMusic == true && numberOfLevels == 7 {
        ...
        if bugs >= 10 {
            print("\(bugs) bugs to fix!")
        } else if hasMusic == false{
            print("Add music!")
        } else if numberOfLevels <= 5 {
            print("Add more \(6 - numberOfLevels) level(s)!")
        }
    }
}
...
func checkTrainingStatus(name: String, bike: Bool, run: Bool) {
    ...
    if bike && run == true {
        print("\(name) is ready!")
    } else if bike == false && run == true {
        print("You can't finish biking!")
    } else if run == false && bike == true {
        print("You can't finish running!")
    } else {
        print("You cant finish both!")
    }
}
